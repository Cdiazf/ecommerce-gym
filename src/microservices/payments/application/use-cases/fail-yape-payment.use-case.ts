...
@Injectable()
export class FailYapePaymentUseCase {
  constructor(
  ...
  async execute(payload: { orderId: string; reason?: string }): Promise<Payment> {
    ...
    const failed: Payment = {
      ...existing,
      status: 'FAILED',
      operationCode: payload.reason ?? existing.operationCode,
      processedAt: new Date().toISOString(),
    };

    const saved = await this.paymentRepository.update(failed);
